...
function init() {
  ...
  obj.addEventListener('touchstart', gestureStart, false);
  obj.addEventListener('touchmove', gestureMove, false);
  obj.addEventListener('touchend', gestureEnd, false);
...
function setActiveHidden() {
  if (!(currentPosition >= maxElements)) {
    let elem = listElNodesObj[0];
    elem.classList.remove('stackedcards-active');
    elem.classList.add('stackedcards-hidden');
    elem.parentNode.removeChild(elem);
    listElNodesObj[currentPosition].classList.add('stackedcards-active');
    if(listElNodesObj[currentPosition].querySelector(".reply")) {
      listElNodesObj[currentPosition].querySelector(".reply").focus();
    }
    if(listElNodesObj[currentPosition].children[0].querySelector(".lang1")) {
      listElNodesObj[currentPosition].children[0].querySelector(".lang1").focus();
    }
  }
};
